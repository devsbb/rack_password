...
module RackPassword
  class Block
    ...
    def call env
      ...
      if request.post? and bv.valid_code?(request.params[@options[:code_param].to_s]) # If post method check :code_param value
        ...
        [
          301,
          {'Location' => request.path, 'Set-Cookie' => "#{@options[:key]}=#{request.params[@options[:code_param].to_s]}; domain=#{domain}; expires=30-Dec-2039 23:59:59 GMT"},
          ['']
        ] # Redirect if code is valid
      else
  ...
  class BlockValidator
    ...
    def valid?
      valid_path? ||
      valid_code?(@request.cookies[@options[:key].to_s]) ||
      valid_ip? ||
      valid_subdomain? ||
      valid_custom_rule?
    end
    ...
    def valid_subdomain?
      return false unless @options.key?(:domains_blacklist)

      parts = @request.host.split('.')
      subdomain = parts[0..-(TLD_LENGTH + 2)].first
      !subdomain || !@options[:domains_blacklist].include?(subdomain)
    end
